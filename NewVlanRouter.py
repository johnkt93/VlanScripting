...
change_number_label = tk.Label(window, text="Change Request Number:")
change_number_label.pack(pady=5)
change_number_input = tk.Entry(window, textvariable=change_number_var)
change_number_input.pack()

router_var = tk.StringVar(window)
router_var.set(routers[0])
router_dropdown_label = tk.Label(window, text="Source Distribution Router")
router_dropdown_label.pack()
router_dropdown = tk.OptionMenu(window, router_var, *routers)
router_dropdown.pack()

destination_switch_frame = tk.Frame(window)
...
switchport_var = tk.StringVar(window)
switchport_label = tk.Label(window, text="Enter the switchport that the end device is connected to:")
switchport_label.pack()
switchport_input = tk.Entry(window, textvariable=switchport_var)
switchport_input.pack()

vlan_number_var = tk.StringVar()
vlan_number_label = tk.Label(window, text="Vlan Number:")
vlan_number_label.pack()
vlan_number_input = tk.Entry(window, textvariable=vlan_number_var)
vlan_number_input.pack()
vlan_name_var = tk.StringVar()
vlan_name_label = tk.Label(window, text="Vlan Name:")
vlan_name_label.pack()
vlan_name_label_input = tk.Entry(window, textvariable=vlan_name_var)
vlan_name_label_input.pack()
vlan_gateway_var = tk.StringVar()
vlan_gateway_label = tk.Label(window, text="Enter the gateway address for the subnet")
vlan_gateway_label.pack()
vlan_gateway_input = tk.Entry(window, textvariable=vlan_gateway_var)
...
vlan_netmask_input = tk.Entry(window, textvariable=vlan_netmask_var)
vlan_netmask_input.pack()
vlan_description_var = tk.StringVar()
vlan_description_label = tk.Label(window, text="Enter a brief description for the vlan")
vlan_description_label.pack()
vlan_description_label_input = tk.Entry(window, textvariable=vlan_description_var)
vlan_description_label_input.pack()

multicast_checkbox_var = tk.IntVar()
multicast_checkbox = tk.Checkbutton(window, text="Check if multicast was requested", variable=multicast_checkbox_var)
